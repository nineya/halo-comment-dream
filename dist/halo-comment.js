...
if (typeof window !== 'undefined') {
  ...
  if (({"NODE_ENV":"production","BASE_URL":"/themes/dream/source/lib/halo-comment@1.0.1/"}).NEED_CURRENTSCRIPT_POLYFILL) {
...
/* harmony default export */ var DreamEmojivue_type_script_lang_js_ = ({
  ...
  props: {
    ...
    url: {
      type: String,
      required: false,
      default: `${"/themes/dream/source/lib/halo-comment@1.0.1/"}assets/emoji/`
    }
  },
  computed: {
    dreamSrc() {
...
function renderedEmojiHtml(html) {
  ...
  for (let emoji of emojiData) {
    let name = emoji.name;
    let img = `<img class="dream-emoji" src="${"/themes/dream/source/lib/halo-comment@1.0.1/"}assets/emoji/${emoji.fileName}.png" alt="${name}"/>`;
